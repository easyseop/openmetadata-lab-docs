...
content("그래서 내린 판단","테스트 결론",
 [("처음의 규칙 기반 · AI 최소화 전략을 실제로 돌려본 결과 —",0,DARK),
  ("잘 통한 것 — 정해진 틀 안에서는 잘 통했다 (1.13.0→1.13.1은 충돌 0으로 깔끔히 통과)",0,GREEN),
  ...
  ("① 같은 효과를 내는 지시(프롬프트·MD)로 주고, ② 과정마다 정해진 형식으로 결과를 받는다",1,DARK),
  ("자유서술로 “했다”고 적게 두지 않는다는 뜻. 결국 하네스 방식이 필요하다는 걸 다시 확인한 셈이다",1,GRAY),
  ("최종 목표 — ①②를 과정 단위로 굳혀 ‘스킬’(정해진 작업 절차)로 만들고, 그걸 묶어 OpenMetadata 운영 하네스 템플릿을 갖추는 것",0,BLUE),
  ("버전업 때마다 새로 짜는 게 아니라 템플릿을 꺼내 쓰는 상태 — 다음 버전에도, 다른 시스템에도 같은 절차가 돈다. 아직 만들지 않았다",1,GRAY)],TEAL,8)
