...
content("강점 — LLM 혼자선 보장 못 하는 것들","왜 ‘룰베이스 검사기’인가",
 [("아래는 전부 LLM도 할 수 있는 일이다. 차이는 능력이 아니라 누가 증언하느냐에 있다 — LLM이 하면 LLM의 말이 되고, 검사기가 하면 증거가 된다",0,TEAL),
  ("빼보기(patch-kill) — 기능이 되는지가 아니라 그 테스트가 진짜인지를 검사한다",0,GREEN),
  ...
  ("한계 — 아무것도 안 보는 테스트는 걸러내지만, 덜 보는 테스트는 못 걸러낸다 (status_code==200만 봐도 통과). 그래서 사람이 읽을 것은 불변식 한 줄",2,AMBER),
...
content("8/5 공유된 ‘LLM 기반 운영 방식’ (참고)","참고한 다른 접근",
 [("개념 — 규칙·지식 문서·자동화로 LLM이 버전 포팅을 빠르게 수행하는 방식",0,BLUE),
  ...
  ("우리 생각 — 검증이 아니라 방향 제안: 이 접근에 검사기로 ‘각 판단의 근거를 자동으로 뒷받침’하는 층을 더하면 속도(LLM)는 살리고 신뢰(근거)는 끌어올릴 수 있겠다. 경쟁이 아니라 보완",0,TEAL)],BLUE,8)
...
content("LLM이 일하고, 검사기가 근거를 붙인다","방향 전환",
 [("LLM — 판단·제안: 무엇을 바꿀지, 코드가 어디로 옮겨졌을지, 충돌 해결, 필요한 테스트",0,BLUE),
  ...
  ("핵심 규칙 — LLM이 ‘됐다’고 적어도 증거가 없으면 ‘미확인’으로 표시한다. 증거 없다고 곧장 ‘거짓’으로 몰지 않는다 → 확인·반박·미확인·사람검토 4단계",0,TEAL)],TEAL,9)
...
content("커스텀 하나(BANK-OM ID)가 검증되는 과정","도입 제안 ① — 검증되는 과정",
 [("제안 — 하이브리드에서 LLM 작업의 ‘증거 층’으로 이 검사기를 도입한다 (자랑이 아니라 도입 대상)",0,TEAL),
  ...
  ("6페이지 한계와의 연결 — “관리 파일이 많다”는 부담을 그대로 두지 않는다. 이 산출물을 사람이 아니라 LLM이 생성·갱신하고, 검사기는 검증만 한다",0,TEAL)],TEAL,10)
...
mf_slide("LLM이 만들고, 사람이 확인한다  (관리 파일 9종)","도입 제안 ② — LLM이 만든다",MF_A,BLUE,11,
...
mf_slide("사람이 정하고, LLM이 못 바꾼다  (관리 파일 5종)","도입 제안 ③ — LLM이 못 바꾼다",MF_B,TEAL,12,
 "다 덜어내도 이 둘은 남긴다 — ① 손대면 안 되는 구역(sensitive-zones) ② 검사한 코드와 승인을 못 바꾸게 묶는 것(candidate-locks)")
mf_slide("검사기가 자동으로 남긴다  (관리 파일 4종)","도입 제안 ④ — 검사기가 남긴다",MF_E,GREEN,13,
...
content("검사기의 정확한 위치 — LLM 위에 얹는 안전장치","오해 방지",
 [("검사기는 LLM을 대신하지 않는다. 검사도 판단도 LLM이 먼저 한다 — 검사기는 그 결과를 한 번 더 확인할 뿐이다",0,TEAL),
  ...
  ("검사기를 ‘LLM보다 똑똑한 판정자’로 쓰면 실패한다 — 그건 이미 해봤다. ‘LLM이 한 일을 한 번 더 확인하는 안전장치’로 쓰면 부담은 작고 얻는 건 크다",0,GREEN)],TEAL,15)

# 16 결론
content("결론","한 장 요약",
 [("작은 변화 — 하위 패치엔 규칙 기반이 그대로 유효. 충돌 0·자동 병합 (4·5p)",0,GREEN),
  ("큰 변화 — 1.13.2처럼 구조가 바뀌면 오판(리로케이션) + 관리 파일 ~25개 운영 부담(6p) → 한계",0,RED),
  ("그래서 — 규칙 기반은 버리지 않는다. 예측 가능 범위엔 그대로, 불확실할 때만 하이브리드. 그 관리 산출물은 LLM이 만들고 검사기는 검증만 (9·10p)",0,BLUE),
  ("① 방식 통제 — 규칙으로 못 박던 것을 지시(프롬프트·MD)로 옮겨 AI가 일하는 방식을 통제 (브랜치·커밋·범위)",0,TEAL),
  ...
  ("현재: 1.13.1 9/9 · 1.13.2 부분 / 다음: 주장–증거 자동 대조기",2,GRAY)],TEAL,16)
